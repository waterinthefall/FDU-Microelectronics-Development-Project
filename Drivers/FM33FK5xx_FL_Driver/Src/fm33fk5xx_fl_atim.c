...
#define IS_ATIM_INSTANCE(TIMx)                     (((TIMx) == ATIM0)||\
                                                    ((TIMx) == ATIM1))
...
#define IS_FL_ATIM_SLAVE_MODE(__VALUE__)           (((__VALUE__) == FL_ATIM_SLAVE_MODE_PROHIBITED)\
                                                    ...
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_RISE_RST_RUN)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_HIGH_RISE_RUN)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_ENCODER_CLK_DIR_X2)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_ENCODER_CLK_DIR_X1)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_ENCODER_CLKWithDIR_X2)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_TRGI_ENCODER_CLKWithDIR_X1)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_QUADRATURE_ENCODER_X1_TI1FP1)\
                                                    || ((__VALUE__) == FL_ATIM_SLAVE_MODE_QUADRATURE_ENCODER_X1_TI2FP2))
...
#define IS_FL_ATIM_BDTR_FILTER(__VALUE__)           (((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1) \
                                                     ...
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N12) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N16) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N24) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N32) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N48) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N64) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N80) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N96) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N128) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N160) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N192) \
                                                     || ((__VALUE__) == FL_ATIM_BREAK_FILTER_DIV1_N256))
...
/** @defgroup TIM_FL_Private_Functions TIM Private Functions
  * @{
  */
static FL_ErrorStatus  ATIM_OCConfig(ATIM_Type *TIMx, uint32_t Channel, FL_ATIM_OC_InitTypeDef *TIM_OC_InitStruct);
/**
  * @}
  */
...
FL_ErrorStatus FL_ATIM_Init(ATIM_Type *TIMx, FL_ATIM_InitTypeDef *TIM_InitStruct)
{
    ...
    if(TIMx == ATIM0)
    {
        FL_CMU_SetATIM0ClockSource(TIM_InitStruct->clockSource);
        FL_CMU_EnableGroup4BusClock(FL_CMU_GROUP4_BUSCLK_ATIM0);
        FL_CMU_EnableGroup1OperationClock(FL_CMU_OPCLK_ATIM0);
    }
    else if(TIMx == ATIM1)
    {
        FL_CMU_SetATIM1ClockSource(TIM_InitStruct->clockSource);
        FL_CMU_EnableGroup4BusClock(FL_CMU_GROUP4_BUSCLK_ATIM1);
        FL_CMU_EnableGroup1OperationClock(FL_CMU_OPCLK_ATIM1);
    }
    /* 设置重复计数值 */
    ...
    while((!FL_ATIM_IsActiveFlag_Update(TIMx)) && i)
    {
        i--;
    }
    /*清除UIF标志，防止产生UG事件中断*/
    FL_ATIM_ClearFlag_Update(TIMx);
    return FL_PASS;
}
...
void FL_ATIM_StructInit(FL_ATIM_InitTypeDef *TIM_InitStruct)
{
    ...
    TIM_InitStruct->clockSource           = FL_CMU_ATIM0_CLK_SOURCE_APBCLK;
...
FL_ErrorStatus  FL_ATIM_IC_Init(ATIM_Type *TIMx, uint32_t channel, FL_ATIM_IC_InitTypeDef *IC_InitStruct)
{
    ...
    /* 通道输入选择 */
    FL_ATIM_SetTISource(TIMx, IC_InitStruct->ITSelect, channel);
    /*捕获极性 */
...
FL_ErrorStatus  FL_ATIM_ETR_Init(ATIM_Type *TIMx, FL_ATIM_ETR_InitTypeDef *TIM_InitStruct)
{
    ...
    /* ETR的输入信号选择 */
    FL_ATIM_SetETRSource(TIMx, TIM_InitStruct->ETRInput);
    /* 外部时钟极性 */
...
FL_ErrorStatus FL_ATIM_OC_Init(ATIM_Type *TIMx, uint32_t channel, FL_ATIM_OC_InitTypeDef *TIM_OC_InitStruct)
{
    ...
    /* 输出比较模式寄存器配置 */
    ATIM_OCConfig(TIMx, channel, TIM_OC_InitStruct);
    /* 手动触发更新事件，将配置值写入 */
    FL_ATIM_GenerateUpdateEvent(TIMx);
    while((!FL_ATIM_IsActiveFlag_Update(TIMx)) && i)
    {
