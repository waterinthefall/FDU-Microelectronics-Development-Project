...
#define IS_EXTI_ALL_INSTANCE(INSTANCE)             (((INSTANCE) == FL_GPIO_EXTI_LINE_0)||\
                                                    ...
                                                    ((INSTANCE) == FL_GPIO_EXTI_LINE_19)||\
                                                    ((INSTANCE) == FL_GPIO_EXTI_LINE_20)||\
                                                    ((INSTANCE) == FL_GPIO_EXTI_LINE_21))
...
static const pSetExtiLineFunc setExtiLineFuncs[] =
{
    ...
    FL_GPIO_SetExtiLine19,
    FL_GPIO_SetExtiLine20,
    FL_GPIO_SetExtiLine21
};
...
FL_ErrorStatus FL_EXTI_CommonInit(FL_EXTI_CommonInitTypeDef *EXTI_CommonInitStruct)
{
    ...
    /* 使能并配置外部中断时钟源 */
    FL_CMU_EnableGroup1OperationClock(FL_CMU_OPCLK_EXTI);
...
{
    /* 关闭外部中断时钟源 */
    FL_CMU_DisableGroup1OperationClock(FL_CMU_OPCLK_EXTI);
    return FL_PASS;
}
...
FL_ErrorStatus FL_EXTI_Init(uint32_t extiLineX, FL_EXTI_InitTypeDef *EXTI_InitStruct)
{
    ...
    /* 设置中断线触发边沿 */
    setTrigEdgeFuncs[extiLineId / 16](GPIO, 1 << (extiLineId % 16), EXTI_InitStruct->triggerEdge);
    /* 延时需要大于5个32K的周期 */
    for(i = 0; i < temp; i++)
    {
        __NOP();
    ...
    /* 清除中断挂起 */
    NVIC_ClearPendingIRQ(EXTIA_IRQn);
    NVIC_ClearPendingIRQ(EXTIB_IRQn);
    NVIC_ClearPendingIRQ(EXTIC_IRQn);
    NVIC_ClearPendingIRQ(EXTID_IRQn);
    NVIC_ClearPendingIRQ(EXTIE_IRQn);
    NVIC_ClearPendingIRQ(EXTIF_IRQn);
...
FL_ErrorStatus FL_EXTI_DeInit(uint32_t extiLineX)
{
    ...
    FL_GPIO_ClearFlag_EXTI(GPIO, extiLineX);
    /* 中断线触发边沿禁止 */
    setTrigEdgeFuncs[extiLineId / 16](GPIO, 1 << (extiLineId % 16), FL_GPIO_EXTI_TRIGGER_EDGE_DISABLE);
    /* 禁止数字滤波 */
